...
import 'repository/allergy_group_repository.dart';
...
class AllergyListPage extends StatefulWidget {
  ...
  @override
  _AllergyListPageState createState() => _AllergyListPageState();
}

class _AllergyListPageState extends State<AllergyListPage> {
  List<SelectionAllergyListData> selectionData = [];
  late AllergyGroupsRepository _allergyGroupsRepository;
  ...
  void initState() {
    super.initState();
    selectionData = [];
    _allergyGroupsRepository = AllergyGroupsRepository(context, widget);
    getAllergies();
  }

  callAddAllergyGroupApi() async {
    _allergyGroupsRepository.addAllergyGroup(widget.name, selectionData);
  }

  callEditAllergyGroupApi() async {
    _allergyGroupsRepository.editAllergyGroup(
        ...
        if (model.success!) {
          if (model.data!.isNotEmpty) {
            for (AllergyListData data in model.data!) {
              ...
              selectionData.add(SelectionAllergyListData(
                  data.sId!, data.name!, data.description!, isSelected));
            }
          }
        }
      } catch (e) {
...
class SelectionAllergyListData {
  late String id, name, price;
  late bool isSelected;

  SelectionAllergyListData(this.id, this.name, this.price, this.isSelected);
}
